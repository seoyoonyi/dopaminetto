...
코드 주석과 JSDoc은 필요한 경우에만 작성하고, `~한다`, `~처리한다`, `~관리한다`처럼 선언형 문체를 사용합니다.

- `~합니다`, `~됩니다`, `~주세요` 같은 존대말은 코드 주석에서 사용하지 않습니다.
- UI 문구와 사용자-facing 메시지는 예외로 존대말을 사용할 수 있습니다.

```typescript
// ❌ 금지 — 코드 주석에 존대말 사용
/**
 * 현재 탭을 활성 타운 탭으로 등록하고 lock 생명주기를 관리합니다.
 */
...
컴포넌트 및 커스텀 훅을 설계할 때 다음 원칙을 참고합니다. 단, 확장 가능성을 이유로 현재 필요하지 않은 추상화를 만들지 않습니다.
...
- `children` 합성처럼 이미 필요한 확장 지점만 열어 둡니다.
...
- UI 컴포넌트는 Props, 상태 관리 스토어, 커스텀 훅 중 현재 코드에서 이미 쓰는 경계를 우선 사용합니다.
